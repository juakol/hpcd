(function() {
    ...
    e.AmRectangularChart = e.Class({
        ...
        measureMargins: function() {
            ...
            d.left && f < c && (this.marginLeft = Math.round(-f + c), !isNaN(this.minMarginLeft) &&
...
(function() {
    ...
    e.AmSerialChart = e.Class({
        ...
        getStartTime: function(a) {
            ...
            return e.resetDateToMin(new Date(a), b.minPeriod,
                1, b.firstDayOfWeek).getTime()
        },
        ...
        zoomScrollbar: function() {
            ...
            if (a) {
                ...
                b.parseDates && !b.equalSpacing ? a.timeZoom(this.startTime,
                    this.endTime) : a.zoom(this.start, this.end)
            }
        ...
        updateChartCursor: function() {
            ...
            if (a) {
                ...
                if (a.valueLineBalloonEnabled)
                    ...
                    for (f = 0; f < this.valueAxes.length; f++) h =
                        this.valueAxes[f], h.balloon && (h.balloon = null);
        ...
        update: function() {
            ...
            this.chartScrollbar &&
                this.chartScrollbar.update && this.chartScrollbar.update();
        ...
        handleCursorZoomReal: function(a, b, c, d, e) {
            var h = e.target,
                f, k;
            this.rotate ? (isNaN(a) || isNaN(b) || this.relativeZoomValueAxes(this.valueAxes, a, b) && this.updateAfterValueZoom(), h.vZoomEnabled && (f = e.start, k = e.end)) : (isNaN(c) || isNaN(d) || this.relativeZoomValueAxes(this.valueAxes, c, d) && this.updateAfterValueZoom(), h.hZoomEnabled && (f = e.start,
                k = e.end));
                    ...
                    if (g = this.chartData[g]) {
                        ...
                        if (b.oneBalloonOnly && b.valueBalloonsEnabled) {
                            ...
                            for (h = d.length - 1; 0 <= h; h--)
                                if (f = d[h], f.balloon.enabled &&
                                    f.showBalloon && !f.hidden) {
                                    k = f.valueAxis.id;
        ...
        handleCursorPanning: function(a) {
            ...
            if (b.zoomable && (0 < Math.abs(d) || 0 < Math.abs(h)))
                ...
                } else if (f = this.start0, g = this.end0, c = g - f, d = Math.round(c * d), h = Math.round(c * h), l = this.chartData.length - 1, a || (c = 0), a = e.fitToBounds(f - d, 0, l - c), c = e.fitToBounds(g - h, c, l), this.start !=
        ...
        arrangeBalloons: function(a) {
            ...
            for (c = 0; c < f; c++) {
                ...
                var b = d.bottom,
                    k = d.bottom - d.yPos;
                0 < c && b - k < e + 3 && d.setBounds && (d.setBounds(0, e + 3, h, e + k + 3), d.restorePrevious(),
                    d.draw());
                d.set && d.set.show();
                e = d.bottom
            }
    ...
    e.Cuboid = e.Class({
        ...
        draw: function() {
            ...
            if (0 < Math.abs(h) || 0 < Math.abs(f))
                if (isNaN(t)) z = q, q = e.adjustLuminosity(B, -.2), q = e.adjustLuminosity(B, -.2), w = e.polygon(b, [0, h, d + h, d, 0], [0, f, f, 0, 0],
                    ...
                    K ? (N = g / 2, q = h / 2, P = g / 2, O =
                        d + h / 2, H = Math.abs(g / 2), G = Math.abs(h / 2)) : (q = d / 2, N = f / 2, O = d / 2, P = g + f / 2 + 1, G = Math.abs(d / 2), H = Math.abs(f / 2));
                    I = G * t;
                    J = H * t;
    ...
    e.CategoryAxis = e.Class({
        ...
        draw: function() {
            ...
            if (e.ifArray(a)) {
                ...
                if (this.parseDates && !this.equalSpacing) this.lastTime = a[a.length - 1].time, this.maxTime = e.resetDateToMin(new Date(this.lastTime + 1.05 * u), this.minPeriod,
                    ...
                    if (this.cellWidth = this.getStepWidth(f), f < k && (k = f), h += this.start, this.stepWidth = this.getStepWidth(f), 0 < k)
                        for (q = Math.floor(f / k), t = this.chooseMinorFrequency(q), f = h, f / 2 == Math.round(f / 2) && f--, 0 > f && (f = 0), w = 0, this.widthField && (f = this.start, q = 1), this.end - f + 1 >= this.autoRotateCount && (this.labelRotationR = this.autoRotateAngle), b = f; b <= this.end + 2; b++) {
                            ...
                            B && (a = NaN, this.ignoreAxisWidth || !c.autoMargins) && (a = "right" == this.position ?
                                c.marginRight - this.titleWidth : c.marginLeft - this.titleWidth, a -= this.tickLength + 10);
                    ...
                    r && 1 < v && (t =
                        this.chooseMinorFrequency(v), e.getPeriodDuration(g, t));
                ...
                for (b = l = 0; b < this.data.length; b++)
                    if (t = this.data[b]) this.parseDates && !this.equalSpacing ? (m = t.time, d = this.cellWidth, "MM" == this.minPeriod &&
                        (d = 864E5 * e.daysInMonth(new Date(m)) * this.stepWidth, t.cellWidth = d), m = Math.round((m - this.startTime) * this.stepWidth + d / 2)) : m = this.getCoordinate(b - h), t.x[this.id] = m;
                if (this.widthField)
                ...
                for (b = 0; b < y; b++)
                    if (l = this.guides[b], q = q = q = r = d = NaN, m = l.above, l.toCategory && (q = c.getCategoryIndexByValue(l.toCategory),
        ...
        adjustBalloonCoordinate: function(a, b) {
            ...
            if (this.stickBalloonToCategory) {
                ...
                if (d) {
                    ...
                    this.rotate ? ("left" == this.position ?
                        (f && (h -= d.width), 0 < h && (h = 0)) : 0 > h && (h = 0), d.fixHLine(a, h)) : ("top" == this.position ? (f && (h -= d.height), 0 < h && (h = 0)) : 0 > h && (h = 0), d.fullWidth && (a += 1), d.fixVLine(a, h))
                }
            }
